...
//
// Title: Driver class to run BoardingQueue program
...
public class BoardingSystemDriver {
  ...
  protected void dequeuePassenger() {
    ...
    if (boardingQueue.isEmpty()) {
      ...
      if (dequeuedPassenger.isCheckedIn()) {
        System.out.println("Boarding " + dequeuedPassenger);
        passengersOnBoard.add(dequeuedPassenger);
      } else {
        System.out.println(dequeuedPassenger.name() + " is not checked in. Skipping.");
      }
    }
  }
      ...
      try {
        switch (command.charAt(0)) {
          ...
          case '2': // [2] Peek next passenger to board"
            System.out.println("Next passenger to board: " + boardingQueue.peekBest());
