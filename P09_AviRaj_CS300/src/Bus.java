...
//
// Title: Main Bus class to include in the nodes in the Binary Search Tree
...
public class Bus implements Comparable<Bus> {
  ...
  public int compareTo(Bus other) {
      ...
      int timeComparison = this.arrivalTime.compareTo(other.arrivalTime);
      if (timeComparison != 0) {
          return timeComparison;
      }

      // If arrivalTime is the same, compare by routeAndTripName
      int routeComparison = this.route.getRouteAndTripName().compareTo(other.route.getRouteAndTripName());
      if (routeComparison != 0) {
          return routeComparison;
      }

      // If routeAndTripName is also the same, compare by direction
      int directionComparison = this.route.getDirection().compareTo(other.route.getDirection());
